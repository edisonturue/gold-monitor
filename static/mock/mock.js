...
// Bars (366 entries each)
var I=null, D=null;
function ci() {
  if(I) return I;
  I=[]; var p=BASE;
  for(var i=365;i>=0;i--) { p+=rf(-14,14); p=Math.max(1950,Math.min(2800,p));
    var o=p-rf(-2,2), h=Math.max(o,p)+rf(0,5), l=Math.min(o,p)-rf(0,5);
    var d=new Date(); d.setDate(d.getDate()-i);
    I.push({ts:d.toISOString(), open:+(o.toFixed(2)), high:+(h.toFixed(2)), low:+(l.toFixed(2)), close:+(p.toFixed(2)), volume:ri(1e3,5e4)}); }
  return I;
}
function cd() {
  if(D) return D;
  var b=BASE*FX/31.1034768; D=[]; var p=b;
  for(var i=365;i>=0;i--) { p+=rf(-6,6); p=Math.max(500,Math.min(650,p));
    var o=p-rf(-1,1), h=Math.max(o,p)+rf(0,2), l=Math.min(o,p)-rf(0,2);
    var d=new Date(); d.setDate(d.getDate()-i);
    D.push({ts:d.toISOString(), open:+(o.toFixed(2)), high:+(h.toFixed(2)), low:+(l.toFixed(2)), close:+(p.toFixed(2)), volume:ri(1e3,3e4)}); }
  return D;
}
// Eager init
ci(); cd();
...
// Indicators
function mks(bars) {
  var c=bars.map(function(b){return b.close;}), len=c.length;
  function ma(n){
    var r=[];
    for(var i=0;i<len;i++){if(i<n-1){r.push(null);continue;}var s=0;for(var j=0;j<n;j++)s+=c[i-j];r.push(+(s/n).toFixed(2));}
    return r;
  }
  function rsi(){
    var r=[];
    for(var i=0;i<len;i++){if(i<14){r.push(null);continue;}
      var g=0,ls=0;for(var j=i-13;j<=i;j++){var d=c[j]-c[j-1];if(d>0)g+=d;else ls-=d;}
      r.push(ls===0?100:+(100-100/(1+(g/14)/(ls/14))).toFixed(2));
    }
    return r;
  }
  function macd(){
    function em(n){
      var r=[],k=2/(n+1);
      for(var i=0;i<len;i++){if(i<n-1){r.push(null);continue;}
        var v=c[i-n+1];for(var j=i-n+2;j<=i;j++)v=c[j]*k+v*(1-k);r.push(+(v.toFixed(2)));}
      return r;
    }
    var e12=em(12),e26=em(26),m=[],sig=[],hst=[];
    for(var i=0;i<len;i++){
      if(e12[i]===null||e26[i]===null){m.push(null);sig.push(null);hst.push(null);}
      else{var mv=+(e12[i]-e26[i]).toFixed(2);m.push(mv);
        if(i<8){sig.push(null);hst.push(null);}else{
          var sv=m.slice(i-8,i+1).reduce(function(a,b){return a+b;},0)/9;
          sig.push(+(sv.toFixed(2)));hst.push(+(mv-sv).toFixed(2));
        }
      }
    }
    return {macd:m, signal:sig, hist:hst};
  }
  var ma5=ma(5),ma20=ma(20),ma60=ma(60),rsi14=rsi(),md=macd();
  return {symbol:"XAUUSD", timeframe:"1d", series:{ma5:ma5,ma20:ma20,ma60:ma60,rsi14:rsi14,macd:md.macd,signal:md.signal,hist:md.hist}, latest:{timestamp:bars[len-1].ts,ma5:ma5[len-1],ma20:ma20[len-1],ma60:ma60[len-1],rsi14:rsi14[len-1],macd:md.macd[len-1],signal:md.signal[len-1],hist:md.hist[len-1],trend_label:ma5[len-1]>ma20[len-1]?"bullish":"bearish",confidence:72,reasons:["MA多头排列","RSI中性偏强"]}};
}

var INDI=mks(I), INDD=mks(cd());

// Prices
function ltp(){
  var ip=BASE+rf(-15,15), uc=FX+rf(-.02,.02), dp=ip*uc/31.1034768, ic=ip-rf(-6,6), dc=dp-rf(-3,3);
  return [{symbol:"XAUUSD",market:"international",price:+ip.toFixed(2),currency:"USD",unit:"盎司",timestamp:ia(ri(2,20)),source:"gold_api_xau",prev_close:+ic.toFixed(2),change:+(ip-ic).toFixed(2),change_pct:+((ip-ic)/ic*1e4).toFixed(2),high:+(ip+rf(2,8)).toFixed(2),low:+(ip-rf(2,8)).toFixed(2),age_sec:ri(2,20),freshness_status:"live",last_changed_at:ia(ri(2,20)),expected_update_sec:10},
   {symbol:"AUCN",market:"domestic",price:+dp.toFixed(2),currency:"CNY",unit:"克",timestamp:ia(ri(2,20)),source:"gold_api_xau",prev_close:+dc.toFixed(2),change:+(dp-dc).toFixed(2),change_pct:+((dp-dc)/dc*1e4).toFixed(2),high:+(dp+rf(1,4)).toFixed(2),low:+(dp-rf(1,4)).toFixed(2),age_sec:ri(2,20),freshness_status:"live",last_changed_at:ia(ri(2,20)),expected_update_sec:10}];
}
...
function mt(m,u){
  var se=u.split("/").filter(Boolean);
  for(var k in _h){var pt=k.split(":");if(pt[0]!==m)continue;
    var ps=pt.slice(1).join(":").split("/").filter(Boolean);
    if(ps.length!==se.length)continue;
    var ok=true,pr={};
    for(var i=0;i<ps.length;i++){if(ps[i][0]===":"){pr[ps[i].slice(1)]=se[i];}else if(ps[i]!==se[i]){ok=false;break;}}
    if(ok)return {h:_h[k],p:pr};}
  return null;
}
...
reg("GET","/api/insight/chat/history",function(){return j({ok:true,username:"demo",limit:80,messages:[{id:100,role:"user",content:"今天金价为什么涨了？",created_at:id(1)},{id:101,role:"assistant",content:"金价上涨受美元走弱和地缘政治因素推动。美元指数下跌0.3%至103.8，市场对美联储降息预期升温。",created_at:id(1)},{id:102,role:"user",content:"技术面怎么看？",created_at:id(2)},{id:103,role:"assistant",content:"日线MA5>MA20>MA60多头排列，RSI62中性偏强，MACD金叉运行良好。支撑位2360，阻力位2400。",created_at:id(2)}],count:4});});
// === POST ===
...
reg("POST","/api/insight/chat",function(){return j({ok:true,reply:pk(["金价短期偏多。MACD金叉，RSI62，MA5>MA20多头排列。支撑2360阻力2400。","美联储纪要显示通胀朝2%迈进，9月降息概率65%。地缘风险支撑金价。","央行购金：中国5月末黄金储备7280万盎司，环比增15万盎司。"]),sources:[]});});
reg("POST","/api/insight/chat/stream",function(){var txt="根据数据综合分析，当前金价走强主要受：\n1. 美元指数走弱至103.8\n2. 地缘政治不确定性\n3. 技术面多头排列\n短期上涨动能仍然存在。";var enc=new TextEncoder();return new Response(new ReadableStream({start:function(c){var i=0;!function p(){if(i>=txt.length){c.enqueue(enc.encode("data: [DONE]\n\n"));c.close();return;}c.enqueue(enc.encode("data: "+JSON.stringify({type:"delta",content:txt.slice(i,i+5)})+"\n\n"));i+=5;setTimeout(p,30);}();}}),{status:200,headers:{"Content-Type":"text/event-stream","Cache-Control":"no-cache"}});});
...
window.fetch=function(u,i){
  ...
  if(url.indexOf("/api/")<0)return _f.call(window,u,i);
